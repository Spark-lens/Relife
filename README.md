...
安装本地 0.3.0 包：在 VS Code 扩展视图右上角选择“从 VSIX 安装…”，选择 `portfolio_viewer/relife-portfolio-0.3.0.vsix`；或在仓库根目录执行：

```bash
code --install-extension portfolio_viewer/relife-portfolio-0.3.0.vsix --force
```

安装后可点击 Activity Bar 中的 `Re` 图标打开投资组合，并在侧栏管理观察分组、布林候选提醒与飞书通知。VS Code 打开期间，插件会按上海时间在 A 股 09:15/15:15、美股 21:15/次日 05:15 自动更新和检查。
